...
export default function HomeServices() {
  return (
    <section className="py-20 md:py-28 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-gradient-to-bl from-[#a40049]/5 to-transparent rounded-full blur-[80px] md:blur-[100px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-0 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-center lg:justify-between items-center lg:items-end mb-12 md:mb-16 gap-8 text-center lg:text-left px-4 sm:px-0">
          ...
          {/* Action Button - Shows on Desktop */}
          <motion.div
        ...
        <div className="hidden lg:grid grid-cols-3 gap-6 px-4 sm:px-0">
          {services.map((service, index) => (
            <motion.div
              ...
              className="group relative h-[420px] rounded-3xl overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-all duration-300 bg-gray-900"
            >
              <img
                src={service.img}
                alt={service.title}
                className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-40 will-change-transform"
              />
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                <h3 className="text-2xl font-bold text-white mb-3 transform translate-y-8 group-hover:translate-y-0 transition-transform duration-500 will-change-transform">
                  {service.title}
                </h3>
                <div className="overflow-hidden">
                  <p className="text-gray-100 text-sm leading-relaxed opacity-0 transform translate-y-8 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 delay-100 will-change-transform">
                    {service.desc}
                  </p>
                </div>
              </div>
            ...
            <div className="flex gap-4 sm:gap-5 pr-4 sm:pr-5">
              {services.map((service) => (
                <div
                  ...
                  tabIndex={0} // Allows card to receive 'focus' on tap
                  className="relative w-[85vw] sm:w-[340px] md:w-[380px] h-[400px] md:h-[450px] rounded-3xl overflow-hidden shadow-md bg-gray-900 flex-shrink-0 group focus:outline-none cursor-pointer"
                >
                  <img
                    src={service.img}
                    alt={service.title}
                    className="absolute inset-0 w-full h-full object-cover opacity-70 transition-transform duration-700 group-focus:scale-110 will-change-transform"
                  />
                  {/* Background gets darker when tapped so text is highly readable */}
                  ...
                  <div className="absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10">
                    <h3 className="text-2xl md:text-3xl font-extrabold text-white mb-2 transform transition-transform duration-500 ease-out group-focus:-translate-y-2">
                      {service.title}
                    </h3>
                    
                    {/* Zero-Lag Height Animation using CSS Grid - Reveals on Tap (Focus) */}
                    <div className="grid grid-rows-[0fr] group-focus:grid-rows-[1fr] transition-[grid-template-rows] duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]">
                      <div className="overflow-hidden">
                        <p className="text-gray-200 text-sm md:text-base leading-relaxed opacity-0 group-focus:opacity-100 transition-opacity duration-700 delay-100 pt-2">
                          {service.desc}
                        </p>
                      </div>
                    </div>
        ...
        >
          <Link href="/services" className="w-full sm:w-auto">
            <button className="w-full sm:w-auto group relative px-8 py-4 bg-gradient-to-r from-[#a40049] to-[#4d002c] rounded-full font-bold text-white shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3">
              Explore All Services
